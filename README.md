...
```bash
git clone https://github.com/kanzaki/RDRAAgent0.6.5.git
cd RDRAAgent0.6.5
node menu.js
```
...
### モデル別の特徴
- codex：Codex Cliを利用　処理されるがファイルが出力されないときはセキュリティ設定「config.toml」を見直す
- claud：Claude Cliを利用　AGENTS.mdが使えないのでCLAUD.mdからAGENTS.mdを参照
...
- モデル実行設定は `モデル設定.json` で管理します（default は `codex`）。
...
- `RDRA_Knowledge/2_RDRASpec/` - 仕様作成の説明
  ...
  - `23_画面一覧生成.md` - 画面一覧作成方法
  - `24_BUC画面生成.md` - BUC画面作成方法
  - `25_アクター画面生成.md` - アクター画面作成方法
  - `26_画面照会生成.md` - 画面照会作成方法
